...
public class Solution {
    ...
    public class ReturnDiaAndHeight {
		int height;
		int Dia;
...
// 	}
	...
	public static ReturnDiaAndHeight diameterOfBinaryTreeBetter(BinaryTreeNode<Integer> root){
		if(root == null) {
			int height = 0;
			int Dia = 0;
			ReturnDiaAndHeight ans= new ReturnDiaAndHeight();
			ans.height = height;
			ans.Dia = Dia;
			return ans;
		}
		ReturnDiaAndHeight ld = diameterOfBinaryTreeBetter(root.left);
		ReturnDiaAndHeight rd = diameterOfBinaryTreeBetter(root.right);
		int height = 1 + Math.max(ld.height, rd.height);
		int Dia = Math.max(ld.height + rd.height, Math.max(ld.height, rd.height));
